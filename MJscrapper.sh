...
echo -e "________________________________ \nVerification de la présence du fichier ${RED}cookie.txt${NC}"
if [ -f "$cookie_file" ] && [ -s "$cookie_file" ]; then
    echo -e "Le fichier cookie.txt existe et n'est pas vide,on continue!"
...
echo -e "\n_____________________________________\n"
echo -e "-->${RED}Fusion${NC} des fichiers de prompts et de dates en un fichier CSV \n"
paste -d ';' prompts$my_uuid.txt dates$my_uuid.txt > resultats$my_uuid.csv
echo -e "\n_____________________________________\n -->${RED}Sauvegarde${NC} de la compilation prompts+dates dans un fichier resultats$my_uuid.csv"
lignes=$(wc -l resultats$my_uuid.csv | cut -d ' ' -f 1)
echo -e "\n \n \n${RED}SUCCES${NC} nous avons pu récupérer ${RED}${lignes}${NC} prompts! \n"